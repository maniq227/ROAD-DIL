...
def acc_mean_over_time(A: np.ndarray) -> float:
    ...
    for i in range(A.shape[0]):
        ...
        if seen.size:
            accs.append(np.mean(seen))
    return float(np.mean(accs)) if accs else float("nan")


def bwt(A: np.ndarray) -> (float, Dict[int, float]):
...
def forgetting(A: np.ndarray) -> (float, Dict[int, float]):
    ...
    for j in range(K_max):
        pre = A[j:E-1, j]  # exclude final row
...
def fwt(A: np.ndarray, baseline: Dict[int, float] | None = None) -> (float, Dict[int, float]):
    per: Dict[int, float] = {}
    diffs: List[float] = []
    # Use pre-task off-diagonal A[j-1, j]; if missing, compute a fallback from first available row
    for j in range(1, A.shape[1]):
        idx = j - 1
        a_prev = A[idx, j] if idx < A.shape[0] else np.nan
        if math.isnan(a_prev):
            # Fallback: use earliest non-nan in column j as an approximation
            col = A[:, j]
            mask = ~np.isnan(col)
            a_prev = float(col[mask][0]) if mask.any() else np.nan
        if math.isnan(a_prev):
            continue
        if baseline and j in baseline and not math.isnan(baseline[j]):
            b = float(baseline[j])
        else:
            b = A[0, j] if not math.isnan(A[0, j]) else 0.0
        d = float(a_prev - b)
        per[j] = d
        diffs.append(d)
    return (float(np.mean(diffs)) if diffs else float("nan")), per
...
def save_figs(A: np.ndarray, domains: List[str], out_dir: Path) -> None:
    ...
    if HAS_SNS:
        ...
        sns.heatmap(data, annot=True, fmt=".3f", cmap="viridis", vmin=0.0, vmax=vmax)
    else:
...
def main() -> None:
    ...
    ap.add_argument("--inputs_root", type=Path, default=Path("YOLO-GDUMB/optimal_output"))
    ap.add_argument("--domains", nargs="+", default=DOMAINS)
    ap.add_argument("--out_dir", type=Path, default=Path("YOLO-GDUMB/optimal_output/figures"))
    ap.add_argument("--baseline_json", type=Path, default=None)
    args = ap.parse_args()

    rows = load_matrix(args.inputs_root, args.domains)
    if not rows:
        raise FileNotFoundError("No continual_eval_matrix found under inputs_root.")
